...
func execSAdd(db *engine.DB, args [][]byte) (redis.Reply, *engine.AofExpireCtx) {
	...
	return protocol.MakeIntReply(int64(count)), &engine.AofExpireCtx{
		NeedAof:  true,
		ExpireAt: nil,
	}
}
...
func execSCard(db *engine.DB, args [][]byte) (redis.Reply, *engine.AofExpireCtx) {
	...
	return protocol.MakeIntReply(int64(set.Len())), nil
}
...
func execSPop(db *engine.DB, args [][]byte) (redis.Reply, *engine.AofExpireCtx) {
	...
	return protocol.MakeMultiBulkReply(result), &engine.AofExpireCtx{
		NeedAof:  true,
		ExpireAt: nil,
	}
}
...
func getAsSet(db *engine.DB, key string) (set Set.Set, errorReply protocol.ErrorReply) {
...
func getOrInitSet(db *engine.DB, key string) (set Set.Set, inited bool, errReply protocol.ErrorReply) {
	set, errReply = getAsSet(db, key)
	if errReply != nil {
