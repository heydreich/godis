...
func execSet(db *engine.DB, args [][]byte) (redis.Reply, *engine.AofExpireCtx) {
	...
	return &protocol.OkReply{}, &engine.AofExpireCtx{
		NeedAof:  true,
		ExpireAt: nil,
	}
}
...
func execSetEX(db *engine.DB, args [][]byte) (redis.Reply, *engine.AofExpireCtx) {
	...
	if err != nil {
		return &protocol.SyntaxErrReply{}, &engine.AofExpireCtx{
			NeedAof:  false,
			ExpireAt: nil,
		}
	}
...
func execPSetEX(db *engine.DB, args [][]byte) (redis.Reply, *engine.AofExpireCtx) {
	...
	ttl, err := strconv.ParseInt(string(args[1]), 10, 64)
	if err != nil {
		return &protocol.SyntaxErrReply{}, nil
	}
...
func execGet(db *engine.DB, args [][]byte) (redis.Reply, *engine.AofExpireCtx) {
	...
	if value == nil {
		return protocol.MakeNullBulkReply(), &engine.AofExpireCtx{
			NeedAof:  false,
			ExpireAt: nil,
		}
	}

	return protocol.MakeBulkReply(value), &engine.AofExpireCtx{
		NeedAof:  false,
		ExpireAt: nil,
	}
}
...
func execIncrBy(db *engine.DB, args [][]byte) (redis.Reply, *engine.AofExpireCtx) {
	...
	if err != nil {
		return protocol.MakeErrReply("ERR step is not an integer or out of range"), &engine.AofExpireCtx{
			NeedAof:  false,
			ExpireAt: nil,
		}
	}
...
func execStrLen(db *engine.DB, args [][]byte) (redis.Reply, *engine.AofExpireCtx) {
	...
	return protocol.MakeIntReply(int64(len(value))), &engine.AofExpireCtx{
		NeedAof:  false,
		ExpireAt: nil,
	}
}

func GetAsString(db *engine.DB, key string) ([]byte, protocol.ErrorReply) {
...
func doIncrBy(db *engine.DB, key string, by int64) (redis.Reply, *engine.AofExpireCtx) {
	...
	if value != nil {
		...
		if err != nil {
			return protocol.MakeErrReply("ERR value is not an integer or out of range"), &engine.AofExpireCtx{
				NeedAof:  false,
				ExpireAt: nil,
			}
		}
	...
	return protocol.MakeIntReply(by), &engine.AofExpireCtx{
		NeedAof:  true,
		ExpireAt: nil,
	}
}
